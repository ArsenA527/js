...
// --------------------
function saveLog(typeTest, startTime, text, filename) {
  if (filename == "") {
    filename = "BA-SNG-80" + typeTest + "_" + startTime + ".txt";
  }
...
function check(isOnes = true) {
  ...
  switch (step) {
    ...
    case 1: {
      if (isOnes) {
        if (buksng_1.alg_state == 2) {
          ...
          if (script_3.result_error_out.includes("СНГ-ОТКАЗ")) {
            flag = false;
          } else {
            flag = true;
          }
    ...
    case 31: {
      ...
      saveLog(typeTest, startTime, finalReport, fileName);
      break;
    }
    default: {
      break;
